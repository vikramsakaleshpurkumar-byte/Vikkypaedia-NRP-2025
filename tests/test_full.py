...
with sync_playwright() as pw:
    ...
    check("retention criterion met", crit[1]['met']=='1' and crit[1]['v']=='24/24')
    check("applied criterion not yet met", crit[2]['met']=='0')
    check("all 24 units mastered", "24 of 24" in p.evaluate("()=>document.querySelector('#dashLeft').textContent"))
    ...
    check("attempt recorded", p.evaluate("()=>JSON.parse(localStorage.getItem('vkp.nrp2025.v1')).exam.attempts.length===1"))
    check("24h lock applied", p.evaluate("()=>JSON.parse(localStorage.getItem('vkp.nrp2025.v1')).exam.lockUntil>Date.now()"))
    ...
    check("verification code present", "NR25-" in p.evaluate("()=>document.getElementById('printRoot').textContent"))
    ...
    check("export units collapsed", exp.count('data-open="0"')>=24)
    check("export keeps all 24 units", exp.count('<section class="unit"')==24)
    ctx.close()
    ...
    check("exported file renders 24 units", p2.evaluate("()=>document.querySelectorAll('.unit').length")==24)
    check("exported file starts fresh (0 mastered)", "0 of 24" in p2.evaluate("()=>document.querySelector('#dashLeft').textContent"))
