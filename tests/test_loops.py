...
with sync_playwright() as pw:
    ...
    print("\n== CONFIDENCE ==")
    check("one confidence bar per checkpoint", p.evaluate("()=>document.querySelectorAll('.q .conf').length===document.querySelectorAll('.q').length && document.querySelectorAll('.q').length===48"))
    ...
    m = (pp or {}).get("modules", {}).get("nrp2025", {})
    ...
    check("export reloads with exactly one confidence bar per item", r["conf"] == r["wired"] == 48, r)
